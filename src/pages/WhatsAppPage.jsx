...
function parseRowsToRecipients(rows = []) {
  return rows
    .map((row) => {
      const name = row.name || row.fullName || row.studentName || row.guestName || row.Name || row.NAME || '';
      const mobile = row.mobile || row.phone || row.number || row.whatsapp || row.Mobile || row.Phone || row.Number || row.WhatsApp || '';
      return {
...
export default function WhatsAppPage() {
  ...
  const toggleRecipient = (mobile) => {
    setSelectedRecipients((prev) => prev.map((item) => item.mobile === mobile ? { ...item, checked: item.checked === false ? true : false } : item));
  };

  const handleInvitationField = (key, value) => setInvitationForm((prev) => ({ ...prev, [key]: value }));
  ...
  return (
    <>
      <PageHeader title="WhatsApp Center" subtitle="Connections, approved templates, quick send and invitation bulk sending from one screen." chips={[{ label: `${connections.length} Connections` }, { label: `${templates.length} Templates` }, { label: `${messages.length} Logs` }, { label: `${selectedCount} Selected`, color: 'secondary' }]} />
      <Alert severity="info" sx={{ mb: 2 }}>Use <strong>entry_pass</strong> to send image invitations to students, parents, team members, volunteers, guests, one number, CSV or Excel.</Alert>
      <Card sx={{ mb: 2 }}><CardContent><Tabs value={tab} onChange={(_, v) => setTab(v)} variant="scrollable" scrollButtons="auto"><Tab value="connections" label={`Connections (${connections.length})`} /><Tab value="templates" label={`Templates (${templates.length})`} /><Tab value="send" label="Quick Send" /><Tab value="invite" label="Invitation Send" /><Tab value="logs" label={`Logs (${messages.length})`} /></Tabs></CardContent></Card>
      {tab === 'send' ? <Card><CardContent><Stack spacing={2}><Typography variant="h6" fontWeight={800}>Quick Send</Typography><Grid container spacing={2}><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="To" value={sendForm.to} onChange={(e) => setSendForm({ ...sendForm, to: e.target.value })} /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth select label="Template" value={sendForm.templateName} onChange={(e) => setSendForm({ ...sendForm, templateName: e.target.value })}><MenuItem value="">No Template</MenuItem>{templates.map((t) => <MenuItem key={t._id} value={t.name}>{t.displayName || t.name}</MenuItem>)}</TextField></Grid><Grid size={{ xs: 12, md: 4 }}><Button variant="contained" fullWidth sx={{ height: '100%' }} onClick={sendText}>Send</Button></Grid></Grid><TextField fullWidth label="Text Message" multiline minRows={4} value={sendForm.text} onChange={(e) => setSendForm({ ...sendForm, text: e.target.value })} /></Stack></CardContent></Card> : null}
      ...
      {tab === 'invite' ? (
        <Stack spacing={2}>
          <Card>
            <CardContent>
              <Stack spacing={2}>
                <Typography variant="h6" fontWeight={800}>Invitation Send</Typography>
                {resultMessage ? <Alert severity={resultMessage.type}>{resultMessage.text}</Alert> : null}
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Event Name" value={invitationForm.eventName} onChange={(e) => handleInvitationField('eventName', e.target.value)} /></Grid>
                  <Grid size={{ xs: 12, md: 2 }}><TextField fullWidth label="Date" value={invitationForm.date} onChange={(e) => handleInvitationField('date', e.target.value)} /></Grid>
                  <Grid size={{ xs: 12, md: 2 }}><TextField fullWidth label="Time" value={invitationForm.time} onChange={(e) => handleInvitationField('time', e.target.value)} /></Grid>
                  <Grid size={{ xs: 12, md: 2 }}><TextField fullWidth select label="Recipients" value={invitationForm.recipientMode} onChange={(e) => handleInvitationField('recipientMode', e.target.value)}>{recipientModeOptions.map((option) => <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>)}</TextField></Grid>
                  <Grid size={{ xs: 12 }}><TextField fullWidth label="Venue" value={invitationForm.venue} onChange={(e) => handleInvitationField('venue', e.target.value)} /></Grid>
                </Grid>

                <Card variant="outlined"><CardContent><Stack spacing={2}><Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', md: 'center' }}><Button variant="contained" component="label" startIcon={<UploadFileIcon />}>Upload Invitation Image<input hidden type="file" accept="image/*" onChange={(e) => uploadInvitationImage(e.target.files?.[0])} /></Button><TextField fullWidth label="Header Image URL" value={invitationForm.imageUrl} onChange={(e) => handleInvitationField('imageUrl', e.target.value)} />{uploadingImage ? <Box sx={{ minWidth: 160 }}><LinearProgress /></Box> : null}</Stack>{invitationForm.imageUrl ? <Box component="img" src={invitationForm.imageUrl} alt="Invitation Preview" sx={{ width: 180, maxWidth: '100%', borderRadius: 2, border: '1px solid', borderColor: 'divider' }} /> : null}</Stack></CardContent></Card>

                {invitationForm.recipientMode === 'single' ? (
                  <Grid container spacing={2}>
                    <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Guest Name" value={invitationForm.singleName} onChange={(e) => handleInvitationField('singleName', e.target.value)} /></Grid>
                    <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="WhatsApp Number" value={invitationForm.singleNumber} onChange={(e) => handleInvitationField('singleNumber', e.target.value)} /></Grid>
                  </Grid>
                ) : null}

                {['csv', 'excel'].includes(invitationForm.recipientMode) ? (
                  <Card variant="outlined"><CardContent><Stack spacing={1.5}><Typography fontWeight={700}>{invitationForm.recipientMode === 'csv' ? 'CSV Upload' : 'Excel Upload'}</Typography><Typography variant="body2" color="text.secondary">Use columns like <strong>name</strong> and <strong>mobile</strong>. Other supported phone headers: phone, number, whatsapp.</Typography><Button variant="contained" component="label" startIcon={<UploadFileIcon />}>Choose File<input ref={fileInputRef} hidden type="file" accept={invitationForm.recipientMode === 'csv' ? '.csv' : '.xlsx,.xls'} onChange={handleSheetUpload} /></Button>{fileName ? <Chip label={fileName} /> : null}</Stack></CardContent></Card>
                ) : null}
                ...
                {invitationForm.recipientMode !== 'single' ? (
                  <Card variant="outlined">
                    <CardContent>
                      <Stack spacing={1.5}>
                        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={1}><Typography fontWeight={700}>Selected Recipients</Typography><Typography variant="body2" color="text.secondary">Tap to include or exclude before sending.</Typography></Stack>
                        <Grid container spacing={1.5}>
                          {selectedRecipients.map((item) => (
                            <Grid key={`${item.mobile}-${item.source}`} size={{ xs: 12, md: 6, xl: 4 }}>
                              <Card variant="outlined" sx={{ borderColor: item.checked === false ? 'divider' : 'primary.main' }}>
                                <CardContent sx={{ pb: '16px !important' }}>
                                  <Stack direction="row" spacing={1} alignItems="flex-start">
                                    <FormControlLabel control={<Checkbox checked={item.checked !== false} onChange={() => toggleRecipient(item.mobile)} />} label="" sx={{ mr: 0 }} />
                          ...
                          {!selectedRecipients.length ? <Grid size={{ xs: 12 }}><Alert severity="warning">No recipients loaded for this source yet.</Alert></Grid> : null}
                        </Grid>
                      </Stack>
                    </CardContent>
                  </Card>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} justifyContent="space-between" alignItems={{ xs: 'stretch', md: 'center' }}>
                  <Typography color="text.secondary">Template: <strong>entry_pass</strong> · Total to send: <strong>{selectedCount}</strong></Typography>
      ...
      {tab !== 'send' && tab !== 'invite' ? <CollectionSection {...current} viewMode={viewMode} setViewMode={setViewMode} onAdd={openAdd} onEdit={openEdit} /> : null}
      {tab !== 'logs' && tab !== 'send' && tab !== 'invite' ? <ResponsiveDialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md"><DialogContent sx={{ pt: 3 }}><Stack spacing={2}><Typography variant="h6" fontWeight={800}>{editing ? `Edit ${current.title}` : `Add ${current.title}`}</Typography><Grid container spacing={2}>{current.formFields.map((field) => <Grid key={field.key} size={{ xs: 12, md: field.multiline ? 12 : 6 }}>{field.type === 'select' ? <TextField fullWidth select label={field.label} value={form[field.key] ?? ''} onChange={(e) => setForm({ ...form, [field.key]: e.target.value })}>{(field.options || []).map((option) => <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>)}</TextField> : <TextField fullWidth label={field.label} multiline={field.multiline} minRows={field.multiline ? 3 : undefined} value={form[field.key] ?? ''} onChange={(e) => setForm({ ...form, [field.key]: e.target.value })} />}</Grid>)}</Grid><Stack direction="row" justifyContent="flex-end" spacing={1}><Button onClick={() => setOpen(false)}>Cancel</Button><Button variant="contained" onClick={save}>Save</Button></Stack></Stack></DialogContent></ResponsiveDialog> : null}
    </>
  );
}
